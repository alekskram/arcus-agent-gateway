#!/usr/bin/env python3
"""MEC-57: optional price-history recorder (standalone, no fastmcp).

Snapshot loop over the tradable universe into monthly-rotated parquet
files, plus an idempotent daily rollup:

    python scripts/recorder.py               # loop, $ARCUS_INTERVAL_SEC (default 300s)
    python scripts/recorder.py --once        # single tick, then exit (systemd timer)
    python scripts/recorder.py --limit 5     # debug: only the first 5 symbols
...
# Run directly from a repo checkout: scripts/ is not on sys.path for
# the package dir (same trick as scripts/smoke_api_offline.py).
REPO = Path(__file__).resolve().parent.parent
if str(REPO) not in sys.path:
    sys.path.insert(0, str(REPO))

from arcus_mcp import api, paths  # noqa: E402
...
def main(argv: list[str] | None = None) -> None:
    ...
    ap = argparse.ArgumentParser(
        prog="arcus-recorder",
        description="Arcus price-history recorder (MEC-57): snapshots "
